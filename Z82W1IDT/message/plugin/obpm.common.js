...
Common.Util = {
	...
	getAvatar : function(userId){
		if(!this.cache[userId]){
			$.ajax({
				type: "GET",
				url: $("input[name='contextPath']").val() + "/contacts/getAvatar.action",
				data: {"id":userId},
				async: false,
				dataType: "json",
	...
	calculateTime : function(_time,format){
		var _timeAgo;
		...
		var Year = timeFixDate.getFullYear();  
		...
		if(Common.Util.daysCalc(_time).days >= 7 && format == "Y-m-d H:i"){
			_timeAgo = Year + "-";
			if (Month >= 10){ 
				_timeAgo += Month + "-"; 
			}else{ 
		...
		}else if(Common.Util.daysCalc(_time).days > 2 && Common.Util.daysCalc(_time).days < 7){
			if (Month >= 10){ 
				_timeAgo = Month + "-"; 
			}else{ 
				_timeAgo = "0" + Month + "-"; 
		...
		}else if(Common.Util.daysCalc(_time).days <= 0 && Common.Util.daysCalc(_time).hours <= 0  && Common.Util.daysCalc(_time).minutes > 5){
