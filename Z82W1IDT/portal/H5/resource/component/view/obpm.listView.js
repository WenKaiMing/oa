...
function viewDoc(docid, formid ,signatureExist,templateForm,isEdit,instanceId,nodeId) {
	...
	//判断触发目标是否不存在"listDataTrTd"的class 并存在onclick事件, 满足则阻止冒泡和默认事件并返回
	if(!$target.hasClass("listDataTrTd") && $target.attr("onclick") && $target.attr("onclick") != ""){
...
function approveLimitapproveLimit(docId,approveLimit){
...
function refresh4Record(){
	...
	if(resourceid!=null && resourceid!=''){
		if (typeof (window.parent.Main) === "object"
		&& typeof (window.parent.Main.renderMenu.refreshTotalRow) === "function") {
			window.parent.Main.renderMenu.refreshTotalRow(resourceid);
		}
	}
}
	...
	$.fn.obpmListView = function(){
		return this.each(function(){
			...
			toDataTdHtml = function($tdField,row){
				 ...
				 * 根据字段类型处理结果字段
				 */
				var processResultByField = function(tdAttrs, result){
					var pHtml = "";
					if(tdAttrs.fieldType=="weixinrecordfield" && jQuery.trim(result).indexOf("{")==0){
						...
						pHtml += convert2HTMLEncode(result);
					}
					return pHtml;
				};
				/**
				 * 根据字段类型处理结果字段2--膏药--重构时和processResultByField合并
				 */
				var processResultByField2 = function(tdAttrs, result){
					var tdHtml = "";
					if(tdAttrs.fieldInstanceOfWordField && jQuery.trim(result).indexOf("{")==0){
						var base64 = new Base64();
						var btnHtml = "<img src='../../share/images/view/word.gif'";
						btnHtml += " onclick=\"showWordDialogWithView('"+tdAttrs.showword+"','WordControl','"+docId+"','"+base64.encode(result)+"','"+tdAttrs.colFieldName+"',3,2,false,true)\" ></img>";
						tdHtml += btnHtml;					
					}else if(tdAttrs.fieldInstanceOfgenericWordField && jQuery.trim(result).indexOf("{")==0){//通用word控件
				...
				if(tdAttrs.displayType){
					...
					if(!(tdAttrs.isReadonly || tdAttrs.colType == "COLUMN_TYPE_LOGO" || tdAttrs.colType == "COLUMN_TYPE_OPERATE") && result.indexOf("<TABLE>") == -1){
						...
						if(!tdAttrs.fieldInstanceOfWordField && !tdAttrs.fieldInstanceOfMapField && !tdAttrs.fieldInstanceOfgenericWordField){
							...
							if(result.indexOf("<TABLE>") != -1){
								...
								if(tdAttrs.colType == "COLUMN_TYPE_FIELD"){
									pHtml += processResultByField(tdAttrs, result);
								}else{
									pHtml += convert2HTMLEncode(result);
								}
							}
...
//									aHtml += "<div style=\"cursor:pointer;\" onclick=\"javaScript:viewDoc('";
								...
								if(result.indexOf("img") != -1) {
									...
									if(result.indexOf("<TABLE>") != -1){
										...
										if(tdAttrs.colType == "COLUMN_TYPE_FIELD"){
											aHtml += processResultByField(tdAttrs, result);
										}else{
					...
					//操作列
					if("COLUMN_TYPE_OPERATE" == tdAttrs.colType){
						if(ColumnOperaType.BUTTON_TYPE_DELETE == tdAttrs.colButtonType){
						...
						}else if(ColumnOperaType.BUTTON_TYPE_DOFLOW == tdAttrs.colButtonType){
							var inputHtml = "<input type=button value='" + tdAttrs.colButtonName;
						...
						}else if(ColumnOperaType.BUTTON_TYPE_TEMPFORM == tdAttrs.colButtonType){
							var inputHtml = "<input type=button value='" + tdAttrs.colButtonName;
						...
						}else if(ColumnOperaType.BUTTON_TYPE_SCRIPT == tdAttrs.colButtonType){
							var inputHtml = "<input type=button value='" + tdAttrs.colButtonName;
						...
						}else if(ColumnOperaType.BUTTON_TYPE_JUMP == tdAttrs.colButtonType){
							var inputHtml = "<input type=button value='" + tdAttrs.colButtonName;
					...
					if(tdAttrs.colType == "COLUMN_TYPE_FIELD"){
						tdHtml += processResultByField2(tdAttrs, result);
					}
					tdHtml += "</td>";
				}
			...
			//根据列文本显示方式的配置，截取文本
			var truncationText = function(input,displayType,displayLength,isShowIcon){
			...
			var $tableHtml = jQuery("<table class=\"table table-bordered table-hover " + Setting.TABLE_CLASS + "\" id=\"dataTable\" style=\"table-layout:auto\"></table>");
			...
			$field.children().children().each(function(i){//行<tr>
				...
				if(i == 0){//首行（列头）
					...
					//首列
					var tdHtml = "";
					tdHtml = "<td style=\"text-align:left;\" class=\"" + Setting.TH_FIRST_TD_CLASS + "\" scope=\"col\"></td>";
					var inputHtml = "";
						inputHtml += "<input style=\"margin-left:11px;\" style=\"margin-left:11px;\" type=\"checkbox\" onClick=\"selectAll(this.checked)\">";
					...
					//首列
					var tdHtml = "<td style=\"text-align:left;\" class=\"" + Setting.TR_FIRST_TD_CLASS + "\">&nbsp;</td>";
					...
					$trField.children().each(function(i){//单元格<td>
						...
						if(sumTdAttrs.isVisible && !sumTdAttrs.isHiddenColumn){
							tdHtml += "<td style='color: #2E6DA4;word-break: break-all;'>";
					...
					var tdHtml =  "<td style=\"text-align:left;\" class=\"" + Setting.TR_FIRST_TD_CLASS + "\">";
						tdHtml += "<input style=\"margin-left:11px;\" type=\"checkbox\" name=\"_selects\" value=\"" + docId + "\"/>";
