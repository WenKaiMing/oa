...
//								+ " align=\"absmiddle\"  title=\""+ monthTitle +"\">"
					...
					$field.find("[subTable='trBody']").each(function(){
						...
						$trBody.find("[subTable='tdBody']").each(function(){
							...
							if (day > 0 && day <= maxIndex) {
								html += "<td class='calendar-day' data-yearindex='" + yearIndex + "' data-monthIndex='" + monthIndex + "' data-day='" + day + "' " +
										"style='border-left:1px solid #e6e6e6;border-top:1px solid #e6e6e6;cursor: pointer;' " +
										"valign='top' onMouseOver=\"this.bgColor='#fefed8';\"";
								html += " onMouseOut=\"this.bgColor='transparent';\" id='cal" + day + "'>";
								...
								html += "</td></tr><tr>";
								html += "<td valign='top' align='left'><div onclick='showTaskContent("+day+");' style='overflow:hidden;'>";
								if(isShowDayInfo){
									$tdBody.find("[moduleType='div1']").each(function(){
							    		...
							    		if(isReadonly == 'true'){
							    			html += "<div style='color:#4664a2;overflow:hidden;width:100%;height: 19px;line-height: 19px;'>";
							    			html += "<a";
							    			if(showTitle == "true")
							    				html += " title='" + list + "'";
							    			html += ">"+ list +"</a></div>";
							    			...
							    			switch(viewType){
							    			case 1:{
							    				html += "<div style='margin:1px;color:#4664a2;overflow:hidden;width:100%;height: 19px;line-height: 19px;'>";
					    ...
					    for(var i = 0; i < 7; i++){
					    	...
					    	if(isShowInfo){
					    		$tdField.find("[moduleType='div1']").each(function(){
					    			...
					    			viewType = parseInt(viewType);
						    		if(isReadonly == 'true'){
						    			html += "<div style='margin:1px;color:#4664a2;overflow:hidden;width:100%;height:100%;'>";
						    			html += "<a";
						    			if(showTitle == "true")
						    				html += " title='" + list + "'";
						    			html += ">"+ list +"</a></div>";
	...
	$("#dataTable").on("click", ".calendar-day", function(){
		var data = $(this).data()
		ShowDayView(data.yearindex+'-'+data.monthindex+'-'+data.day)
	})
