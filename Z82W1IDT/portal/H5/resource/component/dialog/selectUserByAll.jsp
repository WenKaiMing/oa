...
<link rel="stylesheet" href="<s:url value='/portal/share/css/main.css' />" type="text/css">
<style>
#righttitle {
	height: 38px;
    overflow-x: hidden;
...
<script src="<s:url value='/portal/H5/resource/component/showMessage/obpm.showMessage.js'/>"></script><!-- 滚动条 -->
<script src="<s:url value='/portal/H5/resource/script/jquery.slimscroll.min.js'/>"></script>
<script type="text/javascript">
	...
	/*最少选择用户数*/
	var minUser=args['minUser'];
	if(minUser == null || minUser == "" || minUser == "null"){
		minUser = 0;
	}
	/*存放userid*/
			...
			jQuery.ajax({
					...
					success:function(data){
							if(data.trim().length > 0){
								...
								$("#rightcontent").find(".right_content").html("<div class='list_div'>{*[core.dynaform.form.userfield.front.page.tip.can_not_found_user]*}</div>");
								jQuery("#rightcontent").find(".user_run_page").remove();
							}
						},
	...
	function clickRemoveSelect(id,name){
		var $this = jQuery("span#"+id+".onSelect")
		var selectedNum = jQuery("#selectedUserDiv").find('span.onSelect').size();
		if(selectedNum == minUser && minUser > 0){
			OBPM.message.showError('选择数量不能小于 '+minUser);
		}else{
			jQuery(".list_div_click").each(function(){
				if(jQuery(this).attr("id")==id){
					jQuery(this).attr("checked",false);
	...
	function rightContentScroll(){
		...
		$("#rightcontent").find(".right_content").slimscroll({
			height: rightscrollHeight-40,
			color:'#333'
		});
	...
	function renderRight(data){
		...
		var $page = $html.find(".user_run_page");
		$page.css({
			"border-top":"1px solid gray",
			"border-bottom":"none"
		})
		jQuery("#rightcontent").find(".user_run_page").remove();
		jQuery("#rightcontent").append($html.find(".user_run_page"));
		jQuery("#rightcontent").find(".right_content").html($html.html());
	}
	jQuery(document).ready(function(){
		...
		jQuery("#deleteAll").click(function(){			
			...
			jQuery("#selectedUserDiv").find('span.onSelect').each(function(i){
				if(minUser < i + 1){
					$(this).remove();
					jQuery(".list_div_click[id='"+$(this).attr('id')+"']").attr("checked",false);
				}
			})
			resetTargetValue();
		});
		...
		jQuery(".crossUL > li").click(function(){
			...
			$("#rightcontent").find(".right_content").html("");
	...
	<div class="contentDiv on">
		...
		<div id="right" class="crossULdivright">
			...
			<div class="crossULdivright_lef">
				...
				<div id="rightcontent" style="clear: all;"><div class="right_content"></div></div>
			</div>
			<div class="guide"></div>
