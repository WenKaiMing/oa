...
showsubmenuById:function(tabid, targetId) {
	var menuitems=this[tabid+"-menuitems"];
 	...
 	if(targetId != ""){ 
 		var noTabShow = true;
 		var $tabTr = $("#" + targetId).parent().parent().parent();
 		$tabTr.find("div:visible").each(function(){
	...
	if (targetitem) {
		...
		if(oContent != null || oContent != undefined){
			if(oContent.style.display !='block'){
				...
				$(targetitem).parent().parent().siblings().find("a").removeClass("current");
				
				//jack for iframe lazy load
				jQuery(oContent).find("[moduleType='IncludedView']").obpmIncludedView();
			}
			//选项卡有隐藏条件时
			if(oContent.getAttribute("isHide") && oContent.getAttribute("isHide")=="true") {
...
//		}
	...
	if($("#_contentTable").getNiceScroll().length > 0){
        $("#_contentTable").getNiceScroll().resize();
    }
},
...
hideMenus: function (tabid, menuids){
	for (var i = 0; i < menuids.length; i++) {
		...
		content.setAttribute("isHide","true");	//脚本隐藏时加属性isHide,为了dispaly:none时区别时初始时还是脚本隐藏
	}
},
...
showMenus: function (tabid, menuids){
	...
	*/
	...
	for (var i = 0; i < menuids.length; i++) {
		...
		var content = document.getElementById("content_" + menuids[i]);
		menu.style.display = "block";
		content.removeAttribute("isHide");	//释放脚本隐藏时添加的属性isHide
		if($(menu).find("a").hasClass("current")){
			content.style.display = "block";
			noTabShow = false;
		}
	}
	...
	var $tabTr = $("#li_" + menuids[0]).parent().parent();
	$tabTr.find("div:visible").each(function(){
		if($(this).find("a").hasClass("current")){
			noTabShow = false;
		}
	...
	//没有tab被选中时 默认显示未隐藏的第一个
	if(noTabShow){
		var $menu = $("#li_" + menuids[0]);
		var $content = $("#content_" + menuids[0]);
		$menu.find("a").removeClass("btn-outlined").addClass("current");
		$menu.show();
		$content.show();
	}
},
