...
function QueryBtn(actId,params) {
	...
	if(typeof QueryBtn._initialized == "undefined"){
		...
		QueryBtn.prototype.getBeforePostData = function(){
			...
			} else if(typeof(isSubGridView) != "undefined") {
				return subGridView.getGridViewParams();
			} else {
		...
		QueryBtn.prototype.doAfter = function(result){
			...
			if(typeof DisplayView == 'object' && DisplayView.getTheView(obj)){//嵌入视图
				...
				if(typeof(isSubGridView) != "undefined") {
					var action = contextPath+"/portal/dynaform/view/displayView.action";
					var form = jQuery('<form method="get"><input name="_viewid" value="' + this.viewId + '" /></form>')
					.attr('action', action);
