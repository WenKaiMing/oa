	...
	if(typeof JumpToBtn._initialized == "undefined"){
		...
		JumpToBtn.prototype.getBeforePostData = function(){
			if(typeof DisplayView == 'object' && DisplayView.getTheView(this.target)){
				...
				if(typeof(isSubGridView) != "undefined"){
					params = subGridView.getGridViewParams();
				}else{
					params = jQuery(document.forms[0]).serialize();
				}
				return params
			};
		}
		...
		JumpToBtn.prototype.doAction = function(){
			...
			case 1 :
				...
				}else if(document.getElementById("viewid") || typeof(isSubGridView) != "undefined"){    //视图类型
					if(document.getElementById("viewid")){ //普通视图
						view_id = document.getElementById("viewid").value
					}
					if(typeof(isSubGridView) != "undefined"){ //网格视图
						params = subGridView.getGridViewParams();
						view_id = params._viewid;
					}
					docid = view_id;
				}
				...
				case 1://弹出层
					...
					break;
				case 2://页签
					parent.Main.renderTabs.addTab(docid,"...", url);
					break;
				case 3://新窗口
					window.open(url);
					break;
