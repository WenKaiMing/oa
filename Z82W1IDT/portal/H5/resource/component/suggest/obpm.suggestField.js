(function($) {
	$.fn.obpmSuggestField = function() {
		return this.each(function(i) {
			...
			var isSubGridView = $field.attr("_subGridView");
			...
			isSubGridView = (isSubGridView == "true");
			value = value ? value : "";
			...
			if(textType.toLowerCase() != TEXT_TYPE_HIDDEN && displayType != PermissionType_HIDDEN){
				var inputHtml = "<input type='text' class='form-control' GridType='suggestField' isCommonFilter='"+isCommonFilter+"' value='" + value + "' discript='"+discript+"' id='" + name + "_show'"
					...
					**/
					...
					$input.typeahead({
						source:function(query,process) {
							...
							var params = "";
							if(isSubGridView){//hotfix 兼容网格视图
								params = $.param(subGridView.getGridViewParams());
							}else{
								params = jQuery("#document_content").serialize();
							}
							var data = params + "&_formid=" + formId + "&__keyword="+query;
			...
			if(displayType == PermissionType_READONLY || TEXT_TYPE_READONLY == textType.toLowerCase()
					|| displayType == PermissionType_DISABLED){
				if(readOnlyShowValOnly == "true"){
					$spanHtml.append("<span id='" + name + "_show1'>" + text + "</span>");
				}
			} 
			var discriptHtml="";
			if(displayType == PermissionType_HIDDEN){
			...
			//hidden input
			var hiddenHtml = "<input type='hidden' value='" + value + "' id='" + fieldId + "' discript='"+discript+"' name='" + name + "'" + "fieldtype = '" + fieldType + "'" + " GridType='suggestField' resetable=true />";
