...
function dataTableSize(){
	...
	var dataTableH = jQuery("#dataTable").outerHeight();
	if(dataTableH >= 400){
		...
		return this.each(function(){
			...
			toDataTdHtml = function($tdField, row){
				...
				if(!tdAttrs.isHidden){
					...
					if(!(tdAttrs.isReadonly || tdAttrs.colType == "COLUMN_TYPE_LOGO") && tdAttrs.result.indexOf("<TABLE>") == -1){
...
//									aHtml += convert2HTMLEncode(tdAttrs.result);
					...
					if(tdAttrs.fieldInstanceOfWordField){
						var btnHtml = "<img src='../../share/images/view/word.gif' onclick=\"showWordDialogWithView('{*[Show]*} {*[Word]*}','WordControl','"+tdAttrs.docId+"','"+tdAttrs.result+"','"+tdAttrs.colFieldName+"',3,2,false,true);event.stopPropagation();\"></img>";
...
var DisplayView = {
	...
	bindEvent : function($this){
		...
		$(".showNextPage").unbind("click").bind("click",function(e){
		...
		$(".showFirstPage").unbind("click").bind("click",function(e){
			var json = DisplayView.span2Json(this);
			json["_currpage"] = 1;
			var url = DisplayView.getAction(this);
			DisplayView.postForm(this,url,json);
		});
		$(".showPreviousPage").unbind("click").bind("click",function(e){
			var json = DisplayView.span2Json(this);
			var _currpage = parseInt(json["_currpage"])-1;
		...
		$(".showLastPage").unbind("click").bind("click",function(e){
			var json = DisplayView.span2Json(this);
			var pageCount = Math.ceil(parseInt(json["_rowcount"])
					/ parseInt(json["_pagelines"]));
		...
		$(".btn-jumppage").unbind("click").bind("click",function(e){
			var json = DisplayView.span2Json(this);
			var pageCount = Math.ceil(parseInt(json["_rowcount"])
					/ parseInt(json["_pagelines"]));
	...
	downloadFile : function(url,json){
		var $form = $('<form method="post" action="'+url+'" id="downloadfile" style="display:none"></form>');
		$form.append(this.json2Textarea(json));
		$('body').append($form).find($form).submit().remove();
	},
	//打印预览
