...
function createInfo(content) {
	return new JsMessage(JsMessage.TYPE_INFO, content);
}
/**
 * 创建成功通知框
 * 
...
function createSuccess(content) {
	return new JsMessage(JsMessage.TYPE_SUCCESS, content);
}
...
function createWarning(content) {
	return new JsMessage(JsMessage.TYPE_WARNING, content);
}
 ...
 * 发送微信图文消息
 * @param touser
 * 		用户账号列表（消息接收者，多个接收者用‘|’分隔）最多支持1000个。特殊情况：指定为@all，则向关注该企业应用的全部成员发送
 * @param title
 * 		消息标题
 * @param description
 * 		消息描述
...
function sendWeixinRichTextMessage(touser,title,description,url,picUrl,domainId,applicationid){
	...
	$MESSAGE.sendRichTextMessage(touser,title,description,url,picUrl,domainId,applicationid);
}
/**
 * 发送微信文本消息
 * @param touser
...
function sendWeixinTextMessage(touser,content,domainId,applicationid){
	$MESSAGE.sendTextMessage(touser,content,domainId,applicationid);
}
/**
* 发送微信消息
* @param json
...
function sendWeixinMessage(json,domainId){
	$MESSAGE.sendMessage(json,domainId);
}
/**
 * 新建问卷问题
 * @param id
 ...
 * @returns 
 */
function setWaterMarkImgAndText(waterMarkImg,waterMarkStr){
	var result = new Packages.net.sf.json.JSONObject();
	if(waterMarkImg!= null && waterMarkImg != ""){
		result.put("waterMarkPicUrl", waterMarkImg);
	}else{
		result.put("waterMarkPicUrl", "");
	}
	if(waterMarkStr != null && waterMarkStr != ""){
		result.put("waterMarkStr", waterMarkStr);
	}else{
		result.put("waterMarkStr", "");
	}
 ...
 * @returns
 */
function doSycnFromLanxin(){
	var domainId = getDomainid();
	var process = getDomainProcess();
	var domain = process.doView(domainId);
	var service = new Packages.cn.myapps.support.lanxin.service.LanxinService();
	var result = service.synchFromLanxin(domain);
}
 ...
 * @returns
 */
function doSycnToLanxin(){
	var domainId = getDomainid();
	var process = getDomainProcess();
	var domain = process.doView(domainId);
	var service = new Packages.cn.myapps.support.lanxin.service.LanxinService();
	var result = service.synch2Lanxin(domain);
}
/**
 * 发送蓝信图文消息
 * @param touser
 ...
 * @throws Exception
 */
function sendLanxinRichTextMessage(touser,title,description,url,picUrl,domainId,applicationid){
 ...
 * @throws Exception
 */
function sendLanxinTextMessage(touser,content,domainId,applicationid){
...
* @throws Exception
*/
function sendLanxinMessage(json,domainId){
