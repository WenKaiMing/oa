...
FCKPanel.prototype.Show = function( x, y, relElement, width, height )
{
	...
	else
	{
		...
		x += oPos.x ;
		//注释重计算y高度用于解决右键菜单位置错误问题,会造成下拉菜单y轴偏移,不影响使用,待升级ckediter一起解决 -- tank	
		//y += oPos.y ;
		var relElementHtmlLength = relElement.getElementsByTagName("html").length
		if(relElementHtmlLength <= 0){
			y += oPos.y ;
		}
		...
		FCKDomTools.SetElementStyles( this._IFrame,
			{
				left	: x + 'px',
				top		: relElementHtmlLength <= 0 ? y + 'px' : (y+24) + 'px'//24px为编辑器上方菜单高度-- tank
			} ) ;
